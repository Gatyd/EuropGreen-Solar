...
class Command(BaseCommand):
    help = 'Crée les produits de base pour les devis'
    ...
    def handle(self, *args, **options):
        ...
        for product_data in base_products:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'✓ Produit créé: {product.name}')
                )
            else:
                existing_count += 1
                self.stdout.write(
                    self.style.WARNING(f'○ Produit existant: {product.name}')
                )

        self.stdout.write(
            self.style.SUCCESS(
                f'\n✅ Terminé: {created_count} créés, {existing_count} existants'
            )
        )
